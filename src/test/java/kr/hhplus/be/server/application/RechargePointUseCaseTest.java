...
public class RechargePointUseCaseTest {
	...
	void recharge_success() {
		...
		RechargePointUseCase sut = new RechargePointUseCase(walletRepository);
		...
		when(walletRepository.hasProcessed(userId, reqId)).thenReturn(false);
		// 잔액 10000원으로 가정
	...
	void recharge_idempotent() {
		...
		// 이미 충전되었음을 가정
		when(walletRepository.hasProcessed(userId, reqId)).thenReturn(true);
		when(walletRepository.findByUserIdForUpdate(userId))
